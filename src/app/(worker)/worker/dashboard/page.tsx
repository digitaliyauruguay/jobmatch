 ...
 * responder indicaciones de empresas. Todas las acciones muestran
 * feedback inline (Procesando → éxito) antes de recargar.
...
import {
  IconClock, IconCheck, IconX, IconFileText,
  IconExternalLink, IconDownload, IconLoader2,
...
type ApplyState = { [jobId: string]: "idle" | "loading" | "success" | "applied" };
type RespondState = { [appId: string]: "idle" | "loading" | "success" };
...
export default function WorkerDashboard() {
  ...
  const [respondState, setRespondState] = useState<RespondState>({});
  ...
  const fetchApplications = async () => {
    ...
    // Marcar jobs ya postulados
    const applied: ApplyState = {};
    data.forEach((a: Application) => { applied[a.job.id] = "applied"; });
    setApplyState((prev) => ({ ...prev, ...applied }));
  ...
  useEffect(() => {
    ...
    fetchJobs();
    fetchApplications();
    fetch("/api/workers/me").then((r) => r.json()).then(setProfile);
    const jobsInterval = setInterval(fetchJobs, 30000);
    const appsInterval = setInterval(fetchApplications, 30000);
    return () => { clearInterval(jobsInterval); clearInterval(appsInterval); };
  }, []);
  ...
  const handleApply = async (jobId: string) => {
    ...
    if (res.ok) {
      setApplyState((prev) => ({ ...prev, [jobId]: "success" }));
      setTimeout(() => {
  ...
  const handleRespondIndication = async (applicationId: string, status: string) => {
    setRespondState((prev) => ({ ...prev, [applicationId]: "loading" }));
    const res = await fetch(`/api/applications/${applicationId}/respond`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status, reason: status === "REJECTED" ? rejectReason[applicationId] : undefined }),
    });
    if (res.ok) {
      setRespondState((prev) => ({ ...prev, [applicationId]: "success" }));
      setTimeout(async () => {
        setRespondState((prev) => ({ ...prev, [applicationId]: "idle" }));
  ...
  const renderApplyButton = (jobId: string) => {
    const state = applyState[jobId] || "idle";
    if (state === "loading") return (
      <div className="flex items-center justify-center gap-2 w-full py-2 bg-jm-card-hover rounded-lg text-sm text-jm-text-secondary">
        <IconLoader2 size={14} className="animate-spin" />Procesando...
      </div>
    );
    if (state === "success") return (
      <div className="flex items-center justify-center gap-2 w-full py-2 bg-jm-green-bg rounded-lg text-sm text-jm-green-light">
        <IconCheck size={14} />¡Postulación enviada!
      </div>
    );
    if (state === "applied") return (
      <div className="w-full py-2 bg-jm-card-hover rounded-lg text-sm text-jm-text-tertiary text-center">
        Ya postulado
      </div>
    );
    return (
      ...
      <div className="flex gap-6 border-b border-jm-border mt-2 mb-6">
        <button onClick={() => setActiveSection("jobs")}
          className={`py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer ${
            activeSection === "jobs" ? "border-jm-magenta text-jm-magenta-light" : "border-transparent text-jm-text-tertiary hover:text-jm-text"
          }`}>
          Ofertas disponibles
        </button>
        <button onClick={() => setActiveSection("applications")}
          className={`py-3 text-sm font-medium border-b-2 transition-colors cursor-pointer ${
            activeSection === "applications" ? "border-jm-magenta text-jm-magenta-light" : "border-transparent text-jm-text-tertiary hover:text-jm-text"
          }`}>
          Mis postulaciones
        </button>
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {jobs.map((job) => (
                  <div key={job.id} className="bg-jm-card border border-jm-border rounded-lg p-5">
                    ...
                    <p className="text-sm text-jm-text-secondary mt-2 line-clamp-2">{job.description}</p>
                    <div className="mt-4">{renderApplyButton(job.id)}</div>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeSection === "applications" && (
          <>
            <div className="flex gap-2 mb-6 flex-wrap">
              <button onClick={() => setAppTab("SELF")}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                  appTab === "SELF" ? "bg-jm-magenta text-white" : "bg-jm-card text-jm-text-secondary border border-jm-border"
                }`}>
                Mis postulaciones
              </button>
              <button onClick={() => setAppTab("INDICATED")}
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors cursor-pointer ${
                  appTab === "INDICATED" ? "bg-jm-magenta text-white" : "bg-jm-card text-jm-text-secondary border border-jm-border"
                }`}>
            ...
            {loadingApps ? (
              ...
              <p className="text-jm-text-tertiary text-sm">
                {appTab === "SELF" ? "Todavía no te postulaste a ninguna oferta." : "Todavía no recibiste indicaciones de empresas."}
              </p>
            ) : (
                  ...
                  <div key={app.id} className="bg-jm-card border border-jm-border rounded-lg p-4">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                      <div>
                        ...
                        <p className="text-sm text-jm-text-secondary mt-0.5">{app.job.company.name} · {app.job.category.name}</p>
                        <p className="text-xs text-jm-text-tertiary mt-1">{MODALITY_LABELS[app.job.modality]}</p>
                      </div>
                      <Badge variant={app.status === "PENDING" ? "cyan" : app.status === "APPROVED" ? "green" : "red"}
                        icon={app.status === "PENDING" ? <IconClock size={13} /> : app.status === "APPROVED" ? <IconCheck size={13} /> : <IconX size={13} />}>
                    ...
                    {appTab === "INDICATED" && app.status === "PENDING" && (
                      <div className="mt-3 pt-3 border-t border-jm-border flex flex-col gap-2">
                        ...
                        <input type="text" placeholder="Motivo si rechazás (opcional)"
                        ...
                        {respondState[app.id] === "loading" ? (
                          <div className="flex items-center gap-2 px-3 py-1.5 bg-jm-card-hover rounded-lg text-sm text-jm-text-secondary">
                            <IconLoader2 size={14} className="animate-spin" />Procesando...
                          </div>
                        ) : respondState[app.id] === "success" ? (
                          <div className="flex items-center gap-2 px-3 py-1.5 bg-jm-green-bg rounded-lg text-sm text-jm-green-light">
                            <IconCheck size={14} />Respuesta enviada
                          </div>
                        ) : (
                          <div className="flex gap-2">
                            <Button variant="approve" icon={<IconCheck size={13} />}
                              disabled={respondState[app.id] === "loading"}
                              onClick={() => handleRespondIndication(app.id, "APPROVED")}>
                              Aceptar
                            </Button>
                            <Button variant="reject" icon={<IconX size={13} />}
                              disabled={respondState[app.id] === "loading"}
